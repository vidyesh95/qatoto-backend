 ...
 *
 * ⚠️ FINDING, not silently worked around: the POST route declares `idempotency()` with NO
 * `{ required: true }` (see `proof-of-effort.routes.ts`, the "Baking the pie" section) —
 * i.e. honour-if-present, not require. For a once-ever irreversible mutation this means a
 * retried request with no `Idempotency-Key` header is not itself blocked by the idempotency
 * layer; the only thing standing between a retry and a second attempt is the
 * `PIE_ALREADY_BAKED` domain check inside `bakePie.service.ts` (exercised below) plus the DB
 * unique index it's backed by. Flagged for follow-up: this route is a strong candidate for
 * `idempotency({ required: true })`, matching `POST /funding-rounds/:roundId/pledges`.
 */
