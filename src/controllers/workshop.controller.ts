...
/**
 * The Virtual Workshop — board, files and chat (R_AND_D_BACKEND_STRUCTURE.md §8, §11d).
 *
 * EVERY ROUTE HERE IS MEMBER-ONLY. There is no public projection of a workshop, and
 * `requireProjectRole(slug, userId, "contributor")` runs before every handler's first
 * read (§8). Failure renders as 404 through the projects mapper — never 403 — so a
 * stranger cannot learn which project slugs exist.
 *
 * THE SERVER-OWNED FIELDS THAT APPEAR IN NO SCHEMA BELOW, and are therefore 422s by
 * `.strict()` rather than silent overwrites (§13's grep list for this section):
 *   rank · position · sizeBytes · contentSha256 · externalHost · storageProvider ·
 *   objectKey · uploadedByMemberId · authorMemberId · sentAt · editedAt · deletedAt ·
 *   removedAt · createdAt · id
 */
