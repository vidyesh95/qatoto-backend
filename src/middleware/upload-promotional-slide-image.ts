 ...
 *
 * Buffers the file in memory (no disk temp) so we can hand the bytes straight to sharp for
 * validation and then Cloudinary. The byte cap here is a CHEAP first gate that stops
...
/**
 * Run multer for the `image` field and translate its failures into our standard error
 ...
 */
export function uploadPromotionalSlideImage(req: Request, res: Response, next: NextFunction): void {
