 ...
 */
export function uploadProjectCover(req: Request, res: Response, next: NextFunction): void {
  memoryUpload.single("cover")(req, res, (uploadError: unknown) => {
    if (!uploadError) {
      next();
      return;
    }
    ...
    if (uploadError instanceof multer.MulterError) {
      if (uploadError.code === "LIMIT_FILE_SIZE") {
        res.status(413).json({
          status: "error",
          statusCode: 413,
          message: "Cover image exceeds the 5 MB size limit.",
        });
        return;
      }
      res.status(422).json({
        status: "error",
        statusCode: 422,
        message: "Invalid cover image upload.",
      });
      return;
    }
